...
from datetime import datetime # <--- ADICIONE ISSO
# -----------------------------------
...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(basedir, 'garanhuns.db')
...
class Avaliacao(db.Model):
    ...
    # Para facilitar o acesso aos nomes
    cliente = db.relationship('Usuario', foreign_keys=[cliente_id], backref='minhas_avaliacoes')
    profissional = db.relationship('Usuario', foreign_keys=[profissional_id], backref='recebidas')
...
def index():
    ...
    filtro_categoria = request.args.get('categoria', '') # <--- NOVO
    ...
    # Filtro por Categoria (Clicou no ícone)
    if filtro_categoria:
        query = query.filter(Usuario.profissao.contains(filtro_categoria))
    ...
    return render_template(
        ...
        termo_atual=termo,
        categoria_atual=filtro_categoria # <--- Passa para o HTML para destacar o ícone
    )
...
def cadastro():
    ...
    # Se for GET, apenas renderiza o template (Removi o código errado aqui)
...
def detalhes_profissional(id):
    ...
    # Busca as avaliações desse profissional
    avaliacoes = Avaliacao.query.filter_by(profissional_id=id).order_by(Avaliacao.data.desc()).all()
    
    # Calcula a média
    soma_notas = sum([a.nota for a in avaliacoes])
    qtd_notas = len(avaliacoes)
    media = round(soma_notas / qtd_notas, 1) if qtd_notas > 0 else 0
...
@app.route('/avaliar/<int:prof_id>', methods=['POST'])
@login_required
def avaliar(prof_id):
    if current_user.tipo != 'cliente' and current_user.tipo != 'admin':
        flash('Apenas clientes podem avaliar profissionais.', 'danger')
        return redirect(url_for('detalhes_profissional', id=prof_id))

    nota = int(request.form.get('nota'))
    comentario = limpar(request.form.get('comentario'))
...
# -----------------------------------
# ÁREA DO ADMINISTRADOR (MODERAÇÃO)
# -----------------------------------
...
def admin_painel():
    ...
    total_reviews = Avaliacao.query.count()

    # Listas para tabelas (Os últimos cadastrados aparecem primeiro)
    usuarios = Usuario.query.order_by(Usuario.id.desc()).all()
    avaliacoes = Avaliacao.query.order_by(Avaliacao.data.desc()).all()

    return render_template('admin.html', 
                           stats={'users': total_users, 'profs': total_profs, 'reviews': total_reviews},
                           usuarios=usuarios,
                           avaliacoes=avaliacoes)
...
def admin_delete_user(id):
    if current_user.tipo != 'admin': return redirect(url_for('index'))
    
    user = Usuario.query.get(id)
    if user:
        # Primeiro deleta as avaliações ligadas a ele para não dar erro no banco
        Avaliacao.query.filter((Avaliacao.cliente_id==id) | (Avaliacao.profissional_id==id)).delete()
...
def criar_admin_padrao():
    ...
    with app.app_context():
        ...
        if not admin:
            ...
            senha_hash = generate_password_hash('mag0453')
            
            novo_admin = Usuario(
                nome='Administrador Master',
                email='admin@garanhuns.com',
                senha=senha_hash,
                tipo='admin',               # Corrigido
